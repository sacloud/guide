...
resource "sakura_vswitch" "private" {
  ...
  description = "Private network switch for secure communication"
}

resource "sakura_vpn_router" "main" {
  name        = "${var.prefix}-vpc-router"
  description = "VPC Router for maintenance access with WireGuard support"
...
# Packet filter for web servers - commented out due to API changes
# resource "sakura_packet_filter" "web_filter" {
#   name        = "${var.prefix}-web-filter"
#   description = "Packet filter for web servers - HTTP/HTTPS + private network"
# }

# Packet filter for database - commented out due to API changes
# resource "sakura_packet_filter" "database_filter" {
#   name        = "${var.prefix}-database-filter"
#   description = "Packet filter for database - private network only"
# }